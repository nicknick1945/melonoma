...
{
    logger->info("вход в фильтр скилетезации ");

    logger->debug("размер фоточки длина:");
    logger->debug(QString::number(image.width()));
    logger->debug("размер фоточки длина:");
    logger->debug(QString::number(image.height()));
    bitmap = Utils::prepareQImageToBitMap(image);
    logger->debug("битовая карта для изображения создана успешно");
    skeletonZhangSuen();
    logger->debug("пройдена скилетизация успешно");
    //добавление н
    ...
    logger->debug("результаты перенесены с битовой карты на изображение успешно");
    logger->info("скилетизация успешно завершена");
}
    ...
    while (flag){
        ...
        for (w = 1; w < bitmap.at(0).size() - 1; w++){
            for (h = 1; h < bitmap.size() - 1; h++){
                ...
                a = tla(bitmap, w, h,p,b);   /* Функция A */
                p1 = p[0] * p[2] * p[6];
                p2 = p[0] * p[4] * p[6];
...
QString* SkiletizationFilter::getDescription(){
